...
class ModelTest(TestCase):
    ...
    def setUp(self):
        Event.objects.create(
            ...
            limit=2000,
            address='shenzhen',
            start_time='2019-12-31 02:18:22')
...
class LoginActionTest(TestCase):
    ...
    def test_login_action_username_password_error(self):
        """测试用户名密码错误"""
        response = self.client.post(
            '/login_action/', {'username': 'ad1', 'password': '123'})
        self.assertEqual(response.status_code, 200)
...
class SignIndexActionTest(TestCase):
    ...
    def test_sign_index_action_phone_error(self):
        """手机号错误"""
        response = self.client.post(
            '/sign_index_action/1/', {"phone": '18611001101'})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'event id or phone error', response.content)

    def test_sign_index_action_event_id_error(self):
        """发布会编号错误"""
        response = self.client.post(
            '/sign_index_action/5/', {"phone": '18611001100'})
